...
dataFile = open("/home/rsk/Documents/Text mining/headline.txt","rb")
data = dataFile.read()
...
for i in range(len(data)):
    texts.append(data[i][2:-2])      # Removing the square brackets present in the text file
...
def getDocs(term,doclist):
    
    output=[]
    term = stemmer.stem(term)
    for i in range(len(doclist)):
        if term in doclist[i]:
            output.append(doclist[i])
            
            
    return output

#%%
getClosestTerm("india",transformer,lsa)
#%%
kClosestTerms(5,"sachin",transformer,lsa)

#%%
a=getDocs("india",texts)
